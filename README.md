...
设计数据和测试程序，比较你的代码和STL在同等条件下的性能，撰写一份report并提交。

你可以关注**每种操作的平均时长**、**每种操作的次数**、**调整平衡的次数**等数据。

- 如果你的性能更优，尝试解释为什么？
- 如果你的性能更劣，尝试分析并找到优化的方向。
  - 常数优化行不行？
  - 是否可以砍掉某些接口的兼容性？
  - 换个数据结构，是否可能在复杂度正确的情况下，提高某些操作的性能？用简短的代码和计时对比，证明之。
  - 你可以考虑的数据结构有：
    - 跳表
    - AVL树
    - B+树（内存层面）
...
**本问考察的是report和代码是否能够佐证你的结论，你需要针对每种操作给出严格的计时&性能分析，但不用和(1)问一样完成大段面向对象的代码。**
...
**本问是传统题目，提交的程序需要包含main，输入/输出格式请参考test/1.in和test/1.out。** 你可以针对数据点的特性，对程序的任何部分进行优化，包括重写main函数、使用面向过程、抛弃部分未使用的特性、离线处理输入等。但是，请注意，作为一个鲁棒的数据结构，你不能提前假设N的大小，即不能使用静态数组。

请找出一种最优的数据结构，想办法让你的代码跑得尽可能快！或许可以了解*性能调优*。

speedtest按照通过测试点的比例给分；前5名将会有图书奖品。
