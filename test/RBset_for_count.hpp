...
private:
    Compare compare{};
    size_t _rotateCount = 0;      // 记录旋转次数
    size_t _colorFixCount = 0;    // 记录颜色调整次数
public:
    typedef Node<T> Node;
    Node *_root = nullptr;
        ...
        while (node != _root && (!node || node->_col == Black)) {
            if (node == parent->_left) {
                ...
                if ((!bro->_left || bro->_left->_col == Black) && 
                    (!bro->_right || bro->_right->_col == Black)) {
                    bro->_col = Red;
                    node = parent;
                    parent = node->_parent;
                    _colorFixCount++; // 记录颜色调整次数
                } else {
                    if (!bro->_right || bro->_right->_col == Black) {
...
public:
    ...
    iterator upper_bound(const T& key) const {// >
        ...
        // 获取旋转次数
    size_t getRotateCount() const { return _rotateCount; }

    // 获取颜色调整次数
    size_t getColorFixCount() const { return _colorFixCount; }
